#!/usr/bin/env bash
# Single documented entry point for schnorr-nonce-forge.
set -euo pipefail
...
IMAGE=schnorr-nonce-forge
CONTAINER=schnorr-nonce-forge
PORT=5000
...
cmd_reliability() {
  ...
  for i in $(seq 1 "$n"); do
    cmd_up
    SECONDS=0
    if cmd_solve >/tmp/schnorr_solve_$i.log 2>&1 && cmd_grade >/tmp/schnorr_grade_$i.json 2>&1; then
      pass=$((pass + 1))
      echo "run $i: PASS (${SECONDS}s)"
    else
      echo "run $i: FAIL (${SECONDS}s) -- see /tmp/schnorr_solve_$i.log /tmp/schnorr_grade_$i.json"
    fi
    cmd_down
  done
  echo "passed $pass / $n"
